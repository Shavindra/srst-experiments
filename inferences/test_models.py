...
for version in experiment_versions:
    ...
    if class_name == 'bike-asphalt':
        class_name = 'bike'
    MODEL_VERSIONS.append({
        'CLASS_NAME': version.split('_')[3],
        'MODEL': f'/home/sfonseka/dev/SRST/srst-dataloader/experiments/UNET/{class_name}/runs/{version}/models/best_iou_model_{version}.pt',
        'IOU_MODEL': f'/home/sfonseka/dev/SRST/srst-dataloader/experiments/UNET/{class_name}/runs/{version}/models/best_mask_iou_model_{version}.pt'
    })
...
def save_predictions(test_results, SAVE_DIR):

    for item in test_results:
        print(item['image_filename'], item['metrics'])
        ...
        plt.savefig(f'{SAVE_DIR}/predicted_{item["image_filename"]}.png')
        ...
        plt.grid(True)
        plt.savefig(f'{SAVE_DIR}/predicted_distribution_{item["image_filename"]}.png')


for item in MODEL_VERSIONS:
    print('Testing class: ', item['CLASS_NAME'])
    ...
    test_dataloader = dl.SRST_DataloaderGray(mask_dir=TEST_DIR, image_dir=IMG_DIR, mask_count=20)
    test_dataset = test_dataloader.dataset

    test_model(model, test_dataloader.data_loader, DEVICE, class_name=CLASS_NAME, model_path=BEST_MODEL)

    CHECKPOINT_VERSION = 'iou' if 'iou' in BEST_MODEL else 'mask'
    SAVE_PARENT_DIR = f'/home/sfonseka/dev/SRST/srst-dataloader/inferences/{MODEL_NAME}/predictions/{CLASS_NAME}'
    SAVE_DIR = f'{SAVE_PARENT_DIR}/{CHECKPOINT_VERSION}'
    os.makedirs(SAVE_DIR, exist_ok=True)

    print('Saving predictions pickle...')

    with open(f'{SAVE_PARENT_DIR}/test_results_{MODEL_NAME}_{CLASS_NAME}.pkl', 'wb') as f:
        pickle.dump(test_results, f)

    print('Saving predictions...')
    save_predictions(test_results, SAVE_DIR)

    # Reset the test_results list
    test_results = []
    activations = {}
