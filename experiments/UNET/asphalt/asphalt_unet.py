...
def train_one_epoch(model, train_loader, criterion, optimizer, device):
    ...
    metric_iou = IoU()  # Initialize IoU metric for binary classification
    metric_accuracy = BinaryAccuracy()  # Initialize accuracy metric for binary classification
...
def eval_model(model, val_loader, criterion, device):
    ...
    metric_eval_iou = IoU()  # Initialize IoU for binary classification (background, asphalt)
    metric_eval_accuracy = BinaryAccuracy()  # Initialize accuracy metric for binary classification
...
for epoch in tqdm(range(EPOCHS), desc='Epochs'):  # tqdm wrapper for epochs
    ...
    writer.add_scalar('Train_Loss', train_loss, logging_step, walltime=now_before)
    writer.add_scalar('Val_Loss', val_loss, logging_step, walltime=now_before)

    writer.add_scalar('Train_IoU', train_metric_iou, logging_step, walltime=now_before)
    writer.add_scalar('Val_IoU', val_metric_iou, logging_step, walltime=now_before)

    writer.add_scalar('Train_Accuracy', train_metric_accuracy, logging_step, walltime=now_before)
    writer.add_scalar('Val_Accuracy', val_metric_accuracy, logging_step, walltime=now_before)
